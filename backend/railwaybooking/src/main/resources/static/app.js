...
// Comprehensive Indian Railway Master Stations Repository
const ALL_INDIAN_STATIONS = [
    "New Delhi",
    "Mumbai",
    "Varanasi",
    "Howrah",
...
// Fallback Train Schedule Database
...
function updateSimulatorRoute() {
    const fromVal = document.getElementById('fromStationSelect')?.value || "ALL";
    const toVal = document.getElementById('toStationSelect')?.value || "ALL";
    ...
    const fromLabel = (fromVal === "ALL") ? "All Origins" : fromVal;
    const toLabel = (toVal === "ALL") ? "All Destinations" : toVal;

    if (simRouteText) simRouteText.textContent = `${fromLabel} ➔ ${toLabel}`;
    if (simStart) simStart.textContent = fromLabel;
    if (simEnd) simEnd.textContent = toLabel;
}
...
/* ==========================================================================
   Dynamic Station Population & Flexible Search Logic
   ========================================================================== */
function populateStationDropdowns() {
    ...
    // Collect all unique stations from active train schedule + master list
    const uniqueStationsSet = new Set(ALL_INDIAN_STATIONS);
    appState.trainsList.forEach(t => {
        if (t.sourceStation) uniqueStationsSet.add(t.sourceStation.trim());
        if (t.source) uniqueStationsSet.add(t.source.trim());
        if (t.destinationStation) uniqueStationsSet.add(t.destinationStation.trim());
        if (t.destination) uniqueStationsSet.add(t.destination.trim());
    });

    const sortedStations = Array.from(uniqueStationsSet).sort((a, b) => a.localeCompare(b));

    const buildOptions = (allLabel) => {
        let html = `<option value="ALL">🌐 ${allLabel}</option>`;
        sortedStations.forEach(stn => {
            html += `<option value="${stn}">${stn}</option>`;
        });
        return html;
    };

    fromSelect.innerHTML = buildOptions("All Stations (Any Origin)");
    toSelect.innerHTML = buildOptions("All Stations (Any Destination)");

    // Restore selected values if valid
    if (currentFrom && Array.from(fromSelect.options).some(o => o.value === currentFrom)) {
        fromSelect.value = currentFrom;
    }
    if (currentTo && Array.from(toSelect.options).some(o => o.value === currentTo)) {
        toSelect.value = currentTo;
    }
}

function handleStationSelectChange() {
    updateSimulatorRoute();
    handleSearchTrains();
}

function setFromStation(station) {
    const select = document.getElementById('fromStationSelect');
    if (select) {
...
function setToStation(station) {
    const select = document.getElementById('toStationSelect');
    if (select) {
        select.value = station;
        updateSimulatorRoute();
        handleSearchTrains();
...
function clearSearchFilters() {
    const fromSelect = document.getElementById('fromStationSelect');
    const toSelect = document.getElementById('toStationSelect');
    if (fromSelect) fromSelect.value = "ALL";
    if (toSelect) toSelect.value = "ALL";
...
function fetchTrains() {
    fetch(`${API_BASE_URL}/trains/all`)
        ...
        .catch(err => {
            console.warn("Using default train list:", err);
            appState.trainsList = DEFAULT_TRAINS;
            populateStationDropdowns();
            handleSearchTrains();
        });
...
function handleSearchTrains(event) {
    ...
    if (fromVal !== "ALL" && toVal !== "ALL") {
        matched = appState.trainsList.filter(train => {
            ...
            return src.includes(fromVal.toLowerCase()) && dest.includes(toVal.toLowerCase());
        });
    } else if (fromVal !== "ALL") {
        matched = appState.trainsList.filter(train => {
            const src = (train.sourceStation || train.source || "").toLowerCase();
            return src.includes(fromVal.toLowerCase());
        });
    } else if (toVal !== "ALL") {
        matched = appState.trainsList.filter(train => {
            const dest = (train.destinationStation || train.destination || "").toLowerCase();
            return dest.includes(toVal.toLowerCase());
        });
    }
    ...
    if (countEl) countEl.textContent = appState.searchResults.length;
    if (summaryEl) {
        if (fromVal !== "ALL" && toVal !== "ALL") {
            summaryEl.textContent = `Showing trains between ${fromVal} ➔ ${toVal}`;
        } else if (fromVal !== "ALL") {
            summaryEl.textContent = `Showing all trains departing from ${fromVal}`;
        } else if (toVal !== "ALL") {
            summaryEl.textContent = `Showing all trains arriving at ${toVal}`;
        } else {
            summaryEl.textContent = `Showing all available train routes across India (${appState.searchResults.length} trains)`;
        }
    }

    renderTrainCards(appState.searchResults);
}
    ...
    if (!trains || trains.length === 0) {
        container.innerHTML = `
            <div class="card" style="padding: 40px; text-align: center; background: var(--card-bg); border-radius: 16px; border: 1.5px dashed var(--border-color);">
                ...
                <h3 style="color: var(--text-main);">No Direct Trains Found For This Route</h3>
                <p style="color: var(--text-muted); margin-top: 4px;">Try selecting other stations or click below to view all scheduled trains.</p>
            ...
            <div class="pnr-journey-timeline">
                ...
                <div class="route-progress-steps">
                    ...
                    <div class="stn-checkpoint">
                        ...
                        <span class="stn-checkpoint-name">Transit Junction</span>
                        <span class="stn-checkpoint-time">Transit: 10:15 AM</span>
                        <span class="stn-platform-badge">Platform 2</span>
                    </div>
                    <div class="stn-checkpoint">
                        <div class="stn-checkpoint-dot"><i class="fa-solid fa-clock"></i></div>
                        <span class="stn-checkpoint-name">Division Stn</span>
